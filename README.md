...
Each tool is its own SEO-targeted page; the pages share one engine and grow into a
cluster over time.
...
1. Check name/domain availability (tidycsv.com or similar) and update the absolute
   URLs in `sitemap.xml`.
2. Add analytics (privacy-respecting, e.g. a simple page-view counter) to measure the
